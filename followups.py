...
AFTER = define_search_period(10) # = '2017/06/10'
BEFORE = None
...
def get_aliases_from_db(user_id):
    ...
    try:
        aliases = list(db.aliases.find({'user_id': 'me'}, {'user_id': False, '_id': False}))[0]['aliases']
    except:
...
def main(msgs):
    ...
    logging.info('built threads object list. Length = {}'.format(len(threads)))
    logging.info('built list of most recent message ids. Length = {}'.format(len(most_recent_msg_ids)))
    ...
    drafts = msgs_req_followups(msgs, most_recent_msg_ids, 3)
    logging.info('number of mesaged outbound w questions and is most recent across threads = {} out of {}'.format(len(drafts), num_msgs_from_me))
    logging.info('--------------------------------------------------------------------------')
    logging.info('---------------------------MESSAGES TO DRAFT FOR--------------------------')
    logging.info('--------------------------------------------------------------------------')
    logging.info(pformat(drafts, depth=2))
